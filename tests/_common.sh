...
SERVER_PORT="$(_get_port dhcp-lab-server)"
CLIENT_PORT="$(_get_port dhcp-lab-client)"
if [[ -z "$SERVER_PORT" || -z "$CLIENT_PORT" ]]; then echo "ERROR: Cannot find VM ports. Are dhcp-lab VMs running?"; exit 1; fi
